...
Description:
 ...
 In the end, besides the above two files,  we will have TDDFT_SPEC folder in the working directory; freq.out, geom, initqp_input, newtonx.sh, split_initcond.log, JOB_AD(folder) and INITIAL_CONDITIONS(folder) in TDDFT_SPEC; gaussian.com and basis in JOB_AD; I1,I2,...In in INITIAL_CONDITIONS.
...
Note:
 When running on ckpt, if iseed=-1, it might generate a super large random number for iseed, which makes newtonx dies. So I change the iseed to a random value within 2^16.
...
#input function isn't same in python2 and python3
    if 1/2==0:
        ...
        myinput=input
    npoints = myinput('Number of initial conditions to be generated (default=300): ')
    npoints = '300' if npoints=='' else npoints
    print('\'<_\': nstats = %s' % npoints) 
    anh_f = myinput('Harmonic frequency factor (default=0.975): ')
    anh_f = '0.975' if anh_f=='' else anh_f
    print('\'<_\': anh_f = %s' % anh_f)
    nstats = myinput('? How many states do you want to calculate: ')
    print('\'<_\': nstats = %s' % nstats)
    method = myinput('? method to use (default=um062x): ')
    method = 'um062x' if method=='' else method
    print('\'<_\': method = %s' % method)
    basis = myinput('? basisset to use (default=6-31+g(d,p)): ')
    basis = '6-31+g(d,p)' if basis=='' else basis
    print('\'<_\': basis = %s' % basis)
    return npoints, anh_f, nstats, method, basis
...
#write xyz file in 'TDDFT_SPEC' directory, will be deleted after changed to nx format.
        ...
        npoints, anh_f, nst, mthd, bs = setinitcond()
...
def newtonx(opt,log):
    try:
        ...
        if os.path.exists(myd1):
            raise SystemExit('%s Exists. Remove and Try it again!' % myd1)
        os.makedirs(myd2)
        shutil.copy(log,myd1+'/freq.out')
        fxyz,fnx = writeinputfiles(opt,myd1,myd2)
        sdo  = 'cd TDDFT_SPEC;module load chem/newtonx/2.0;'
        sdo += '$NX/xyz2nx < '+fxyz+';'
        sdo += '$NX/nxinp < '+ fnx+';'
        sdo += 'rm '+fxyz+' '+fnx+';'
        sdo += 'echo \"\'<_\': Reminder: the job DOES NOT run in a batch system. \n\";'
...
#for every splitting job, change 'iseed = -1' to 'iseed = 1234++'; write a nx_submit.sh with its directory and different jobname(structname+I).
        ...
        with open(myd1+'/newtonx.sh','w') as fo:
            fo.writelines(['cd '+'/'.join([pwd,myd3,I])+'; sbatch -p compute -A cei --time=3-00:00:00 nx_submit.sh\n' for I in allI])
        print('**\(^O^)/**Please check your TDDFT_SPEC folder.To submit all of your jobs do:\n cd TDDFT_SPEC\n bash newtonx.sh')
        print('FYI: 28cores\n  32atoms  16states k initconds ---  0.42x(k+1) h\n\t   20states k initconds ---  0.62x(k+1) h')
        print('  67atoms  25states k initconds ---  5.2x(k+1) h')
        print('  78atoms  14states k initconds ---  7.4x(k+1) h')
        print('           17states k initconds ---  9.5x(k+1) h')
        print('           18states k initconds ---  9.8x(k+1) h')
        print(' 133atoms  13states k initconds ---   48x(k+1) h')
    except:
        err=sys.exc_info()
